...
void exit_game_cb(lv_event_t *e)
{
    ...
    printf("正在删除vm创建的面板\n");
    if (lv_obj_has_flag(lv_obj_get_child(lv_layer_top(), 0), LV_OBJ_FLAG_HIDDEN) == true)
    {
        lv_obj_del(lv_obj_get_child(lv_scr_act(), 0));
        printf("正在删除vm创建的面板n");
    }
    system("kill -9 mplayer");
    system("kill -2 mplayer");
    printf("退出了\n");
    exit(0);
}
...
void pro_max_dis(int num)
{
    ...
    lv_obj_move_foreground(lv_obj_get_child(lv_layer_top(), 0));
    lv_obj_move_foreground(lv_obj_get_child(lv_layer_top(), 1));
}
...
void pro_min_dis(int num)
{
    ...
    if (lv_obj_has_flag(lv_obj_get_child(lv_layer_top(), 0), LV_OBJ_FLAG_HIDDEN) == false)
    {
...
int main(void)
{
    ...
    lv_obj_align(obj_exit, LV_ALIGN_TOP_RIGHT, -5, 15);
    ...
    pthread_mutex_init(&mutex_lv, NULL);
    /*事物处理及告知lvgl节拍数*/
    while (1)
    {
